...
// Add this to src/lib/hubspot/actions.ts - inside the HUBSPOT_ACTIONS object

COMPANY_UPDATER: {
  id: 'COMPANY_UPDATER',
  name: 'HubSpot Company Updater',
  description: 'Update Company data in HubSpot CRM. Update company properties like names, phone numbers, assigned owners, industry, country, etc.',
  configFields: [
    {
      name: 'companyName',
      label: 'Company Name',
    ...
    {
      name: 'properties',
      label: 'Company Properties to Update',
      type: 'multiselect',
      required: true,
      options: [
        { value: 'about_us', label: 'About Us' },
      ...
      placeholder: 'Select company properties to update'
    }
  ],
  ports: {
    inputs: [
      { id: 'input_company_name', label: 'Company Name', type: 'string', isActive: true, isListType: false }
    ],
    outputs: []
  },
  getDynamicPorts: (config: any) => {
    ...
    // Create an input port for each selected property
    const inputPorts = [
      // Always include the company name input
      { id: 'input_company_name', label: 'Company Name', type: 'string', isActive: true, isListType: false }
    ];
    ...
    config.properties.forEach((propertyId: string) => {
      // Find the option to get its label
      const options = HUBSPOT_ACTIONS.COMPANY_UPDATER.configFields.find(f => f.name === 'properties')?.options || [];
    ...
    return {
      inputs: inputPorts,
      outputs: [
        // { id: 'output_updated', label: 'Updated', type: 'boolean', isActive: true, isListType: false },
        // { id: 'output_company_id', label: 'Company ID', type: 'string', isActive: true, isListType: false }
      ]
    };
  }
},

// Add this to the HUBSPOT_ACTIONS object in src/lib/hubspot/actions.ts

CONTACT_UPDATER: {
  id: 'CONTACT_UPDATER',
  name: 'HubSpot Contact Updater',
  description: 'Update Contact data in HubSpot CRM. Update contact properties like names, email, phone, job title, lifecycle stage, etc.',
  configFields: [
    {
      name: 'contactEmail',
      label: 'Contact Email',
    ...
    {
      name: 'properties',
      label: 'Contact Properties to Update',
      type: 'multiselect',
      required: true,
      options: [
        { value: 'city', label: 'City' },
      ...
      placeholder: 'Select contact properties to update'
    }
  ],
  ports: {
    inputs: [
      { id: 'input_contact_email', label: 'Contact Email', type: 'string', isActive: true, isListType: false }
    ],
    outputs: []
  },
  getDynamicPorts: (config: any) => {
    ...
    // Create an input port for each selected property
    const inputPorts = [
      // Always include the contact email input
      { id: 'input_contact_email', label: 'Contact Email', type: 'string', isActive: true, isListType: false }
    ];
    ...
    config.properties.forEach((propertyId: string) => {
      // Find the option to get its label
      const options = HUBSPOT_ACTIONS.CONTACT_UPDATER.configFields.find(f => f.name === 'properties')?.options || [];
    ...
    return {
      ...
      outputs: [
        // { id: 'output_updated', label: 'Updated', type: 'boolean', isActive: true, isListType: false },
        // { id: 'output_contact_id', label: 'Contact ID', type: 'string', isActive: true, isListType: false }
      ]
    };
  }
}

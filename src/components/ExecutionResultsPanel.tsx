...
const renderHubspotUpdaterResults = (nodeId: string, result: any, entityType: 'company' | 'contact') => {
  if (!result.success || !result.data) {
    return (
      <div className="text-red-500">
        {result.error?.message || `Failed to update HubSpot ${entityType}`}
      </div>
    );
  }

  // Handle both company and contact data structures
  const entityName = entityType === 'company' 
    ? result.data.companyName 
    : (result.data.contactName || result.data.contactEmail);
  
  const entityId = entityType === 'company'
    ? (result.data.companyId || result.data.output_company_id)
    : (result.data.contactId || result.data.output_contact_id);
  ...
  // Entity URL (only for contacts)
  // const entityUrl = entityType === 'contact' ? result.data.contactUrl : null;
  
  // Entity identifier label
  const entityIdentifierLabel = entityType === 'company' 
    ? 'Company ID' 
    : 'Contact ID';
  
  // Entity secondary identifier (only for contacts)
  const showSecondaryIdentifier = entityType === 'contact';
  const secondaryIdentifierLabel = 'Contact Email';
  const secondaryIdentifierValue = result.data.contactEmail;
        ...
        {wasUpdated ? (
          <div className="space-y-3">
            <div className="bg-green-50 p-3 rounded flex items-center text-green-700">
              ...
              <span>Successfully updated {entityType} "{entityName}"</span>
            </div>
            
            <div className="space-y-2">
              <div className="font-medium">{entityIdentifierLabel}: {entityId}</div>
              
              {/* Only render for contacts */}
              {showSecondaryIdentifier && secondaryIdentifierValue && (
                <div className="font-medium">{secondaryIdentifierLabel}: {secondaryIdentifierValue}</div>
              )}
              
              {/* Only render for contacts with URL */}
              {/* {entityUrl && (
                <div className="mt-2">
                  <a 
                    href={entityUrl} 
          ...
          <div className="text-amber-600 p-3 bg-amber-50 rounded">
            {entityType === 'company' ? 'Company' : 'Contact'} "{entityName}" found, but no properties were updated
          </div>
        )}
      </div>
    </div>
    ...
    switch (node.type) {
      ...
      case "gdocs":
        ...
        if (node.data.config.action === "COMPANY_READER") {
          ...
          return renderHubspotUpdaterResults(nodeId, result, 'company');
        } else if (node.data.config.action === "CONTACT_UPDATER") {
          return renderHubspotUpdaterResults(nodeId, result, 'contact');
        }
        break;
    }
